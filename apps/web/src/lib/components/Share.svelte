<script lang="ts">
  ...
  messageIndex = 0;
  let nextMessageIndex = $derived((messageIndex + 1) % messages.length);
...
<button onclick={doShare}>
  <svg width="63" height="63" viewBox="0 0 227.216 227.216">
    <path
  ...
  <div class="sharer-viewport">
    <div class="fullbox sharer-messages" class:moving>
      <strong class="fullbox hidden" aria-hidden="true">
        {messages[nextMessageIndex]}
      </strong>
      <strong class="fullbox">{messages[messageIndex]}</strong>
    </div>
  </div>
</button>
...
<style>
  ...
  .sharer-viewport strong {
    display: flex;
    align-items: center;
    overflow: hidden;
  }
  .sharer-messages.moving {
