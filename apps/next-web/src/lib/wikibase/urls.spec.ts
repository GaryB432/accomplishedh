...
describe("Urls", () => {
  ...
  test("queries", () => {
    ...
    expect(thumbnail_query_url(["Q937"], 320)).toEqual(
      "https://www.wikidata.org/w/api.php?action=query&titles=Q937&prop=pageimages&pithumbsize=320&format=json&origin=*",
    );
    expect(thumbnail_query_url(["Q937", "Q1234"], 100)).toEqual(
      "https://www.wikidata.org/w/api.php?action=query&titles=Q937%7CQ1234&prop=pageimages&pithumbsize=100&format=json&origin=*",
    );
  });
});
